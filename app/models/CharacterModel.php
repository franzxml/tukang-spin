...
class CharacterModel
{
    ...
    public function addCharacter(array $data): int
    {
        $query = "INSERT INTO characters 
                  (name, element, weapon_type, rarity, role, equipped_weapon_id, equipped_artifact_set_id, art_sands, art_goblet, art_circlet, level, constellation, talent_na, talent_skill, talent_burst, description, image_url, namecard_url)
                  VALUES 
                  (:name, :element, :weapon_type, :rarity, :role, :equipped_weapon_id, :equipped_artifact_set_id, :art_sands, :art_goblet, :art_circlet, :level, :constellation, :talent_na, :talent_skill, :talent_burst, :description, :image_url, :namecard_url)";
        $this->db->query($query);
    ...
    public function updateCharacter(array $data): int
    {
        $query = "UPDATE characters SET
                    ...
                    description = :description, image_url = :image_url, namecard_url = :namecard_url
                  WHERE id = :id";
        $this->db->query($query);
        $this->bindParams($data);
