 ...
 *
 * Handles database interactions for characters.
 * Updated to support Artifact linking and Compatible Weapons fetching.
 *
 * @package App\Models
 */
class CharacterModel
{
    ...
    {
        // Join with Weapons AND Artifact Sets
        $query = "SELECT c.*, 
                         ...
                         a.name as artifact_name, a.image_url as artifact_image, a.bonuses as artifact_bonuses
                  FROM " . $this->table . " c
                  LEFT JOIN weapons w ON c.equipped_weapon_id = w.id
     ...
     * Used for the Hover Alternative feature.
     */
    public function getCompatibleWeapons(string $type, ?int $excludeId): array
    {
        ...
        $query .= " ORDER BY rarity DESC, base_atk DESC LIMIT 5"; // Limit suggestions to 5 best
